const LOCAL_COLLECTOR_NAME_KEY = "slowpoke_local_collector_name_v2";
...
const NAME_PARTS = {
  objects: [
    "烏龍麵", "粉紅泡泡", "一朵雲", "夕陽", "旅行地圖", "小小月亮",
    "車站便當", "一袋星星", "海風", "水溝蓋圖鑑", "慢吞吞的夢", "剛買的冰淇淋"
  ],
  roles: [
    "發呆研究員", "旅行觀察員", "水溝蓋獵人", "慢慢探險家", "粉紅收藏家",
    "沿路散步員", "呆呆獸調查員", "旅行者", "路邊考察員", "今日值日收藏家"
  ],
  places: [
    "月台上", "雨裡", "水溝蓋旁", "海風吹來的地方", "烏龍麵店門口",
    "夕陽下面", "陌生的小巷裡", "公車站牌旁", "港口邊", "一朵雲下面"
  ],
  placeActions: [
    "發呆", "慢慢走", "等公車", "不小心睡著", "研究路線", "尋找粉紅色",
    "假裝沒有迷路", "看著天空", "整理圖鑑", "決定先休息一下"
  ],
  chosenBy: [
    "粉紅泡泡", "烏龍麵香氣", "呆呆獸", "夕陽", "海風", "路邊的小花",
    "月台廣播", "一場小雨", "旅行地圖", "剛好經過的雲"
  ],
  chosenActions: [
    "選中的", "召喚的", "盯上的", "留下來的", "輕輕推著走的",
    "偷偷認證的", "邀請加入小隊的", "叫去休息的", "帶來這裡的", "認作朋友的"
  ],
  forgottenThings: [
    "趕路", "回家", "撐傘", "今天星期幾", "看時間", "原本要去哪裡",
    "把烏龍麵吃完", "自己正在迷路", "先找出口", "不要一直發呆", "公車幾點來", "拍完照要走"
  ],
};
...
function generateCollectorName() {
  ...
  if (pattern === 0) return `抱著${randomItem(NAME_PARTS.objects)}的${randomItem(NAME_PARTS.roles)}`;
  if (pattern === 1) return `在${randomItem(NAME_PARTS.places)}${randomItem(NAME_PARTS.placeActions)}的${randomItem(NAME_PARTS.roles)}`;
  if (pattern === 2) return `被${randomItem(NAME_PARTS.chosenBy)}${randomItem(NAME_PARTS.chosenActions)}${randomItem(NAME_PARTS.roles)}`;
  return `忘記${randomItem(NAME_PARTS.forgottenThings)}的${randomItem(NAME_PARTS.roles)}`;
}

function getOrCreateLocalCollectorName() {
  const existing = localStorage.getItem(LOCAL_COLLECTOR_NAME_KEY);
  if (existing) return existing;
  const generated = generateCollectorName();
  localStorage.setItem(LOCAL_COLLECTOR_NAME_KEY, generated);
  return generated;
}
