      ...
      try {
        ...
        if (locationName) {
          if (!locationWasEditedByUser) {
            ...
            locationStatus.textContent =
              `已定位：${locationName}｜可直接修改後再儲存`;

            if (locationHint) {
              locationHint.textContent =
                "這只是建議地點；按下收服前，可改成任何你想保存的文字。";
            }
          } else {
            locationStatus.textContent =
              "已取得位置；你已自行修改地點，將保存目前欄位中的文字。";
          }
        } else {
          locationStatus.textContent =
            `位置已記錄：${gpsText}。沒有查到地名，可直接修改。`;
        }
      } catch (error) {
        console.error(error);
        locationStatus.textContent =
